...
read_temp=scipy.io.loadmat('data/data2_snr.mat')
...
for j in range(S):
    for i in range(r2):
        ...
        DF_T_cnn_low = cnn_low.model_(
            torch.tensor(S_est[i, :, :, j].reshape(1, I, 2), dtype=torch.float).permute(0, 2, 1)) \
            .detach().numpy().tolist()
        DF_T_cnn_low = np.array(DF_T_cnn_low)
        DF_T_cnn_low = np.reshape(DF_T_cnn_low, I)

        DF_T_sresnet7 = sresnet7.model_(
            torch.tensor(S_est[i, :, :, j].reshape(1, I, 2), dtype=torch.float, device=torch.device('cpu')).permute(
                0, 2, 1)) \
            .detach().numpy().tolist()
        ...
        DF_T_sresnet7_v2 = sresnet7_v2.model_(
            torch.tensor(S_est[i, :, :, j].reshape(1, I, 2), dtype=torch.float, device=torch.device('cpu')).permute(0, 2,
                                                                                                                 1))[1] \
            .detach().numpy().tolist()
...
plt.ylim([0.1,100])
plt.xlim([-15,15])
plt.savefig('./fig/SNR/SNR.pdf', dpi=200)
plt.show()
